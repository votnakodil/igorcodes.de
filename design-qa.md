...
- Header glass: the established 4.5px blur, 64% page-background opacity, 82% dark-mode saturation, 68% contrast, and 68% brightness remain unchanged. The resume button remains below the header layer.
- Header activation: the header uses native CSS `sticky` positioning. `data-scrolled` enables the glass material only when the Hero content reaches the header. On a fresh load at the absolute top, the page advances by one imperceptible pixel before paint so Safari immediately uses its dark native scroll-edge blur instead of the warmer unscrolled toolbar material.
